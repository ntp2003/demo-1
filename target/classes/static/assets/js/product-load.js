...
const valueTypeProduct=[];
function LoadProduct() {
    $.getJSON("/product/filter", function (json) {
		console.log(json);
                ...
                let productHtml1 = $(`                    
                    ...
                    <div class="product-grid-giay">
						...
						<div class="color-available" style="display:flex;justify-content: center">
                            ...
                            <span class="price">${product.categories[0].price}</span>
                        </div>
                    </div>
				  </div>
                `);
                $("#row-giay").append(productHtml1);
        }); 
    });
};
...
function LoadFilterProduct() {
    $.getJSON("/product/type", function (json) {
        console.log(json);
        var pCatalogList1 = json; // Sửa lại để sử dụng dữ liệu trực tiếp từ json

        // Tạo một category mới ngoài vòng lặp
        let categoryHtml = $(`
            <div class="Category-filter" style="margin-bottom: 15px;border: 2px solid #D6EAF8;  border-radius: 5px;">
                <article class="filter-group">
                    ...
                    <input value="All" type="checkbox" checked="" class="custom-control-input" />
        ...
        `);
        // Thêm category vào filter-container
        $(".filter-container").html(categoryHtml);

        // Thêm từng sản phẩm vào phần card-body của category
/*        pCatalogList1.forEach((product) => {
            let productHtml = $(`
                <label class="custom-control custom-checkbox">
                    <input value="${product.productTypeId}" type="checkbox"  class="custom-control-input" />
                    <div class="custom-control-label">${product.typeName}</div>
                </label>
            `);
            $(".filter-content .card-body").append(productHtml);
        });
$(".filter-container").on("change", ".custom-control-input[value='All']", function () {
        // Nếu checkbox All được chọn
        if ($(this).prop("checked")) {
            // Tắt tất cả các checkbox khác
            $(".filter-content .card-body .custom-control-input[value!='All']").prop("checked", false);
        }
    });

    // Xử lý sự kiện khi checkbox khác được chọn
    $(".filter-container").on("change", ".custom-control-input[value!='All']", function () {
        // Nếu một checkbox khác được chọn, bỏ chọn checkbox All
        $(".filter-content .card-body .custom-control-input[value='All']").prop("checked", false);
    });*/
    // Thêm từng sản phẩm vào phần card-body của category
pCatalogList1.forEach((product) => {
